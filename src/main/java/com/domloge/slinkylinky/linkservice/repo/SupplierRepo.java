...
public interface SupplierRepo extends PagingAndSortingRepository<Supplier, Long>, CrudRepository <Supplier, Long> {
    ...
    @Query(nativeQuery = true,
        ...
        "        where pl.link_demand_id in "+
        "            (select demand.id from link_demand demand where demand.domain = "+
        "                (select demand.domain from link_demand demand where demand.id=?1))) "+      // exclude suppliers that already have a paid link for this domain
        "AND s.DA >= (select demand.da_needed from link_demand demand where demand.id=?1) "+         // match DA
        "AND s.id in (select sc.supplier_id from supplier_categories sc where sc.categories_id in "+ // match categories
        "                (select ldc.categories_id from link_demand_categories ldc where ldc.link_demand_id=?1)) "+
