...
public class Semrush {
    ...
    public SemRushMonthlyData forMonth(String user, String domain, Month month) {
    ...
    public String getTrafficData(String domain, Month monthOfDataRequired) {
        ...
        //     displayOffset += 12;
        // }
        int displayOffset = LocalDate.now().getMonthValue() - monthOfDataRequired.getValue(); 
        if(displayOffset <= 0) {
            displayOffset += 12;
        }
        ...
        String url = sr_base
            +"/?type=domain_rank_history"
            +"&display_limit="+(displayOffset)
            +"&display_offset="+(displayOffset-1)
            +"&key="+semrushKey
            +"&export_columns="+columns
            +"&database=uk"
            +"&domain="+domain;

        ResponseEntity<String> srResp = rest.exchange(url, HttpMethod.GET, null, String.class);
        if(srResp.getStatusCode().isError()) {
            throw new RuntimeException("Error getting data from Semrush: "+srResp.getStatusCode());
        }
        log.trace("SR response: {}", srResp);
        return srResp.getBody();
    }
}
