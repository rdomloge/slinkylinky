...
export default function LeadsIndex() {
    ...
    const [startPage, setStartPage]       = useState('');     // '' = use resume/beginning
    const [scrapeProgress, setScrapeProgress] = useState(null); // { lastOffset, currentPage, pageSize }
    ...
    // When the scrape modal opens, fetch the resume position so the Advanced panel can
    // show the current page and default the "start from page" placeholder.
    useEffect(() => {
        if (!showScrapeModal) return;
        (async () => {
            try {
                const r = await fetchWithAuth('/.rest/leads/scrape/metadata');
                if (!r?.ok) return;
                setScrapeProgress(await r.json());
            } catch { /* non-fatal */ }
        })();
    }, [showScrapeModal]);
    ...
    async function triggerScrape() {
        ...
        const hasStartPage = Number.isInteger(parsedStartPage) && parsedStartPage > 0;
        try {
            const r = await fetchWithAuth('/.rest/leads/scrape', {
                ...
                body: JSON.stringify({
                    ...
                    limit: scrapeLimit,
                    incremental,
                    ...(hasStartPage ? { startPage: parsedStartPage } : {}),
                }),
            });
            setStartPage(''); // consume the one-shot override
                ...
                <Modal title="Scrape Collaborator.pro" dismissHandler={() => setShowScrapeModal(false)} width="w-full max-w-lg" dismissOnBackdropClick={false}>
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="rounded-xl border border-slate-200 bg-slate-50">
                            ...
                            >
                                <span className="text-xs font-semibold text-slate-700">Advanced</span>
                                <svg
                                ...
                                <div className="flex flex-col gap-3 border-t border-slate-200 px-3 py-3">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="text-xs font-semibold text-slate-700">Resume from last position</span>
                                            <span className="text-[11px] text-slate-400">Off = rescan from the top of the catalogue</span>
                                        </div>
                                        <button
                                            type="button"
                                    ...
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="text-xs font-semibold text-slate-700">Start from page</span>
                                            <span className="text-[11px] text-slate-400">
                                                {scrapeProgress
                                                    ? `Currently at page ${scrapeProgress.currentPage} · overrides resume`
                                                    : 'Leave blank to use resume'}
                                            </span>
                                        </div>
                                        <input
                                            type="number"
                                            ...
                                            placeholder={scrapeProgress ? String(scrapeProgress.currentPage) : '1'}
                                            className="w-20 bg-white border border-slate-200 rounded-lg px-2 py-1.5 text-sm text-slate-700 text-center placeholder-slate-300 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
                                        />
                                    </div>
                                </div>
                            )}
